# tg: @Hidlow
# github: https://github.com/hiikikomorii
# советую запускать скрипт с автоматическим закрытием консоли (/c or pause in .bat) чтобы не висел лишний раз

#основные модули, их не трогаем
import subprocess
import sys
import ctypes

# тут надо заменить на ваши импорты (желательно охватывать все возможные модули из проекта, а не только из main файла)
try:
    # пример
    import colorama

    # если все модули пройдут проверку, запускается ваш main файл
    hidlow_path = "Ваш_скрипт.py"
    subprocess.Popen(
    ...
    # в случае если вы используете свои импортируемые модули
    # для того, чтобы проверить целостность файла(ов)
    if e.name == "Ваш.модуль.из.сборки":
        ctypes.windll.user32.MessageBoxW(0, f"Сборка повреждена\nпуть {e.name} не найден\nПроверьте совместимость сборки", "Error", 0x10)
        sys.exit()


    #boottraper запускается только тогда, когда скрипт ловит ModuleNotFoundError
...
вы пытались запустить, запустится без этого меню.
    ...
    # апдейд pip до последней версии
    def upd_pip_cmd():
    ...
    # отдельный поток для того чтобы gui не завис
